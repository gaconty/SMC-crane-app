...
def get_input(prompt, data_type=str, default_value=None):
    while True:
        ...
        try:
            if data_type == float: return float(user_input.replace(',', '.'))
            if data_type == int: return int(user_input)
        except ValueError:
            print(f"  -> Lỗi: Vui lòng nhập số hợp lệ.")

def input_crane_details():
    print("\n" + "="*50)
    print("NHẬP DỮ LIỆU CẨU MỚI (UPDATE: Z-COORDINATES)")
    print("="*50)
    
    crane_id = get_input("  ID Cẩu (viết liền, vd: sany_scc800)", str)
    model_name = get_input("  Tên hiển thị", str)
    max_capacity_ton = get_input("  Sức nâng tối đa (Tấn)", float)

    print("\n[2] CẤU TRÚC GẦM & THÂN MÁY")
    upper_mass_ton = get_input("  Khối lượng bàn quay (Upper - Tấn)", float)
    # NEW: Input Z for Upper
    upper_z_m = get_input("  [MỚI] Cao độ trọng tâm bàn quay (Z - m)", float, default_value=1.5)
